...
// Helper: register or update chairperson account
...
router.get('/requests', protect, async (req, res) => {
  try {
    ...
    const requests  = await DepartmentRequest.find({ skillFaculty: facultyId })
      .populate('skillFaculty', 'name code').sort({ createdAt: -1 });
...
router.put('/requests/:id/approve', protect, async (req, res) => {
  try {
    ...
    if (request.isEdit && request.deptIndex != null) {
      ...
      // ── NEW: Notify the chairperson of this department
      if (existing.chairperson) {
        const chairperson = await Chairperson.findById(existing.chairperson);
        if (chairperson) {
          await notifyStaff({
            ...
            title:  `Department Info Updated — ${dept.name}`,
            message: `Dean ${req.user.name} has updated your department "${dept.name}".\n\nPlease review your department details and verify the information is correct.`,
            type:   'dean_update',
            metadata: { facultyId, deptIndex: request.deptIndex },
          });
        }
      }
    } else {
...
// Dean edits department directly ── triggers Chairperson notification
router.put('/direct/:facultyId/:deptIndex', protect, async (req, res) => {
  try {
    ...
    // ── NEW: Notify the chairperson of this department
    if (dept.chairperson) {
      const chairperson = await Chairperson.findById(dept.chairperson);
      if (chairperson) {
        await notifyStaff({
          recipientId:    chairperson._id,
          recipientModel: 'Chairperson',
          ...
          message: `Dean ${req.user.name} has updated your department "${dept.name}".\n\nPlease review your department details and verify the information is correct.`,
          type:   'dean_update',
          metadata: { facultyId: faculty._id, deptIndex: idx },
        });
      }
    }
