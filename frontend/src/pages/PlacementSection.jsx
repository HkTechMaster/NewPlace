...
export default function PlacementSection({ studentId }) {
  ...
  return (
    <div>
      ...
      {activeView === 'jobs' && (
        !jobs.length ? (
          <div style={{textAlign:'center',padding:'40px',color:'var(--text-muted)'}}>No eligible jobs yet. Check back soon!</div>
        ) : (
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:14}}>
            {jobs.map(job => {
              const appStatus = job.applicationStatus ? statusConfig[job.applicationStatus] : null;
              return (
                <div key={job._id} style={{background:'var(--bg-card)',border:`1px solid ${job.alreadyApplied ? 'rgba(16,185,129,0.3)' : 'var(--border)'}`,borderRadius:'var(--radius)',padding:'18px 20px',display:'flex',flexDirection:'column',gap:10}}>
                  ...
                  {job.alreadyApplied && appStatus ? (
                    ...
                    <button onClick={() => openApplyModal(job)} style={{padding:'9px',borderRadius:'var(--radius-sm)',border:'none',fontFamily:'var(--font-body)',fontSize:'0.875rem',fontWeight:700,cursor:'pointer',background:'var(--success)',color:'white',transition:'all 0.15s'}}>
                      Apply Now →
                    </button>
                  )}
                </div>
      ...
      {activeView === 'offers' && (
        !selectedDrives.length ? (
          <div style={{textAlign:'center',padding:'40px',color:'var(--text-muted)'}}>No offer letters yet. Keep applying!</div>
        ) : (
